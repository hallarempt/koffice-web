<?php
  $title='KOffice 1.1 Release Notes';
  $site_root='../';
  include("koffice.inc");
  include("header.inc");
?>

      <p>The KOffice team is proud to announce the release of KOffice 1.1.</p> 
      <p>Various resources are available.</p>
      <p><h2>Announcement</h2></p>
      <ul>
      <li><a href="/announcements/announce-1.1.phtml">Announcement</a></li>
      </ul>
      <h2>Changelog</h2>
      <ul>
      <li><a href="/announcements/changelog-1.1.phtml">Changelog</a></li>
      </ul>     
      <h2>Download</h2>
      <p>Both source code and binary packages are available.</p>
      <ul>
      <li><a href="/download/source.php">Source code</a></li>
      <li><a href="/download">Binary packages</a></li>
      </ul>
      </p>

<!--       <h2>Bugs</h2>
      <p>A fix for xxx is available. The patch can be applied against the KOffice 1.1 sources.</p>
      <p>The fix is available <a href="/bugfixes/rc1-blah.diff">here</a>.</p>
      <p>The following bugs are fixed:</p>
      <ul>
      <li>blah.</li>
      </ul>
      </p> -->
      
<?php include("footer.inc"); ?>












