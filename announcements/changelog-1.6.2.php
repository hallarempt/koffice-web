...
<h3 id="newfeatures">New Features</h3>
    ...
    <em>Krita</em>
        <ul>
            <li>Halftoning filter</li>
            <li>Smudge painting operation</li>
            <li>add a new colorify filter, this filter allow to apply to a black and white image a new colour</li>
...
<ul>
  ...
  <li>set LC_NUMERIC again to C so that decimal separators are always points (and, for instance, never comma)</li>
...
<ul>
  ...
  <li>the time-value was correct parsed+evaluated but didn't got transferred back what made times not working for all AM/PM definitions (if 12h clock was enabled) (<a href="http://bugs.kde.org/140336">bug 140336</a>). </li>
...
<ul>
  ...
  <li>force to recalculate all variable after loading otherwise link and custom was not re-sized very well</li>
</ul>
...
<h3 id="kexi">Kexi</h3>
    ...
    <ul>
      ...
      <li>improved saving table extended schema data for look-up fields</li>
    ...
    <ul>
      <li>fixed altering table schema when look-up column has been changed</li>
    ...
    <ul>
      ...
      <li>do not show pop-up after clicking on combo box</li>
...
<ul>
  ...
  <li>fix strange Behaviour with Select All and adjustment layer (<a href="http://bugs.kde.org/137638">bug 137638</a>)</li>
  ...
  <li>optimise emboss filter</li>
  <li>update the selection status when using the colour range dialog</li>
  ...
  <li>fix crash when calling the layer property of an adjustment layer which has been moved to the bottom of the stack (<a href="http://bugs.kde.org/138425">bug 138425</a>)</li>
  <li>fix dying of watercolour images</li>
...
<ul>
  ...
  <li>improve progress reporting: the initial right-angle rotations are now also progress reported. So now the user gets feedback immediately</li>
  ...
  <li>fix crashes when clicking with the right mouse button on an unactivated splitview (<a href="http://bugs.kde.org/123500">bug 123500</a>)</li>
</ul>
<em>File import/export</em>
<ul>
  <li>fix crashing when saving twice an image through graphicsmagick filter (<a href="http://bugs.kde.org/137847">bug 137847</a>)</li>
...
<ul>
  ...
  <li>change content size when zoom is changed. This caused problem with embedded formula. Still has some issues, but it works now (<a href="http://bugs.kde.org/136636">bug 136636</a>)</li>
